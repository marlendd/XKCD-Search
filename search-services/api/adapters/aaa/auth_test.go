...
func TestNew(t *testing.T) {
	...
	t.Run("credentials without signing secret", func(t *testing.T) {
		t.Setenv("ADMIN_USER", "admin")
		t.Setenv("ADMIN_PASSWORD", "secret")
		t.Setenv("JWT_SECRET", "")
		_, err := aaa.New(time.Minute, slog.Default())
...
func TestVerify(t *testing.T) {
	...
	t.Run("token signed with another secret", func(t *testing.T) {
		t.Setenv("ADMIN_USER", "admin")
		t.Setenv("ADMIN_PASSWORD", "secret")
		t.Setenv("JWT_SECRET", "another-signing-key")

		other, err := aaa.New(time.Minute, slog.Default())
		assert.NoError(t, err)
		token, err := other.Login("admin", "secret")
		assert.NoError(t, err)
